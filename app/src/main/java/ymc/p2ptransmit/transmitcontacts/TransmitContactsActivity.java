package ymc.p2ptransmit.transmitcontacts;
...
public class TransmitContactsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		final TextView txtPercent = (TextView)findViewById(R.id.txt_trans_contact_percent);
    		...
    		public void handleMessage(Message msg) {
    			switch(msg.what){
					...
					case UPDATE_PROCESS:
						...
						int progress = (100 * count) / totalNum;
						progProcess.setProgress(progress);
        ...
        if("send".equals(cmd))
        {
        	...
        	SendContactsThread sct = new SendContactsThread();
        	sct.start();
        	txtStatus.append("联系人个数为:" + String.valueOf(getContactsNum()) + "\n");
        }
        else
        {
    ...
    public class SendContactsThread extends Thread{
		...
		public void run(){
			...
			try {
				Message.obtain(handler, 0, "等待接收方回应\n").sendToTarget();
	    		Socket msocket = server.accept();
	    		Message.obtain(handler, 0, "建立链接完成，即将开始同步...\n").sendToTarget();
				...
				{
					Message.obtain(handler, 0, "开始同步...\n").sendToTarget();
				}
				else if(recMessage.equals("Refuse"))
				{
					Message.obtain(handler, 3, "对方拒绝接收您的通讯录信息\n").sendToTarget();
					this.interrupt();
				}
			} catch (IOException e) {
				e.printStackTrace();
    ...
    public class RecContactsThread extends Thread
    {
		...
		public void run(){
    		try {
				...
				outputStream.flush();
				Message.obtain(handler, 0, "已经准备好接收通讯录...\n").sendToTarget();
				String message;
