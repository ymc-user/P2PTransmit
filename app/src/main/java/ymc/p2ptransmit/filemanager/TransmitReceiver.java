...
public class TransmitReceiver {
	...
	//接收文件内容
	public FileResult ReceiveData(Map<String,Integer> transmitProgress)
	{
		...
		try{		
			...
			fileResult.setFileName(this.fileName);
			fileResult.setSender(this.SenderName);
			fileResult.setFilePath(savePath);
			fileResult.setResult("来自" + this.SenderName + "的文件" + this.fileName + "接收完成");
			return  fileResult;
		}catch(Exception e){
			fileResult.setResult("接收错误:" + e.getMessage() + "\n");
			return fileResult;
		}
	};
